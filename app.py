...
with st.sidebar.expander("🛠️ OPERACION", expanded=True):
...
try:
    ...
    # Recalculo de Eficiencia y Calor Real
    ...
    with c_left:
        ...
        st.subheader("Balances de Calor (Entradas y Salidas)")
        st.markdown("""
        | Sistema | Fuente de Energia | Calor Cedido [kW] | Calor Aprovechado [kW] |
        | :--- | :--- | :--- | :--- |
        | **Caldera** | Combustible | """ + f"{m_comb*LHV:.2f}" + """ | """ + f"{Q_caldera:.2f}" + """ |
        | **Intercambiador** | Vapor de Agua | """ + f"{Q_HX_real:.2f}" + """ | """ + f"{Q_HX_real:.2f}" + """ |
        | **Pasteurizador** | Agua de Proceso | """ + f"{Q_HX_real*0.95:.2f}" + """ | """ + f"{Q_util:.2f}" + """ |
        """)
    ...
    with c_right:
        ...
        st.subheader("Efecto de T_vapor en la Eficiencia")
        t_range = np.linspace(T_I_water_OUT + 5, T_I_steam_IN + 30, 20)
        q_max_range = C_min * (t_range - T_I_water_IN)
        e_range = Q_HX_real / q_max_range
        
        chart_df = pd.DataFrame({"T_vapor [C]": t_range, "Efectividad": e_range})
        st.scatter_chart(chart_df, x="T_vapor [C]", y="Efectividad", color="#29b5e8")
        st.info(f"Punto actual: T={T_I_steam_IN} C | E={E_real*100:.1f}%")
        st.caption("A menor temperatura de entrada de vapor, la efectividad (E) aumenta al reducirse el potencial maximo (Q_max).")
